// Export all icon categories
export * from './categories/app/index';
export * from './categories/design-tools/index';
export * from './categories/framework/index';
export * from './categories/programming/index';
export * from './categories/tools/index';
export * from './categories/ai/index';

// Export utilities
export * from './types';
export * from './create-icon';
export * from './icon';

// Export registry with Oxycons component and utilities
export { Oxycons, type OxyconsType, type IconName, type OxyconsProps } from './registry';
export { getIconNames, getIconsByCategory, getCategoryInfo, CATEGORIES } from './registry';
